...
describe TargetProcess::Base, vcr: true do
  ...
  describe '.find' do
    ...
    context 'with passed string' do
      it 'raise TargetProcess::BadRequest error' do
        expect do
          subject.find('asd')
        end.to raise_error(TargetProcess::APIError::BadRequest)
      end
    end
  ...
  describe '#respond_to?' do
    it 'does not responds to underscored getter for non existed attribute' do
    ...
    it 'does not responds to camelized getter' do
      expect(subject.new.respond_to?(:camelizedMethod)).to be_false
    end

    it 'does not responds to camelized setter' do
      expect(subject.new.respond_to?(:camelizedMethod=)).to be_false
    end

    it 'does not responds to id setter' do
      expect(subject.new.respond_to?(:id=)).to be_false
    end

    it 'does not responds to bang methods' do
      expect(subject.new.respond_to?(:underscored_method!)).to be_false
    end

    it 'does not responds to question methods' do
      expect(subject.new.respond_to?(:underscored_method?)).to be_false
    end
  end
